...
APP_PATH="${1:-${ZCODE_MACOS_RELEASE_APP_PATH:-/Applications/Lumi Agents.app}}"
# 安装包身份与后端环境分轴：ZCODE_PREVIEW_IDENTITY=1 让生产后端的构建仍是 Lumi Agents Preview。
...
if [ "${APP_PATH:-}" = "--help" ] || [ "${APP_PATH:-}" = "-h" ]; then
  cat <<'USAGE'
Usage:
  bash scripts/doctor-macos-release-app.sh "/Applications/Lumi Agents.app"
  ZCODE_MACOS_RELEASE_APP_PATH="/Applications/Lumi Agents.app" pnpm run doctor:macos-release
