...
| 贡献溯源（DCO）      | 门禁已就位并如实运行；main 现存 **8 个未签名提交**（第 9 节 #11），未代签，等作者本人认证                                                |
...
| 11  | DCO 溯源：main 上 **8 个提交缺少 `Signed-off-by`**（7 个分叉压缩提交，加 PR #8 的 squash 提交 `55eb7a1`——GitHub squash 重写作者并丢弃 trailer） | `check-dco.mjs` 已如实报告；未代签                     | 由真实作者本人补签（`git commit --amend -s` 后 force-push，或由本人重新 squash）；今后合并使用 merge commit 保留签名 | 所有者（作者本人） |
