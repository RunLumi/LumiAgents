...
| 项目                 | 状态                                                                                                                                     |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | --- | -------------- | --------------------------------------------------------- |
| 项目维护主体         | **CLOUDJET SOLUTIONS PTE. LTD.**（Singapore UEN **201708398E**）；负责 Lumi Agents 分支维护与发布，不据此取得上游版权 |
| 第一方许可证         | **Apache-2.0**，保留上游 `LICENSE` 原文（含 `Copyright 2026 Z.AI Co., Ltd`）；Cloudjet 自有/受让 Lumi 内容的权利边界见 `RIGHTS.md` |
...
| 贡献溯源（DCO）      | 门禁已就位并如实运行；main 现存 **9 个未签名提交**（第 9 节 #11），未代签，等作者本人认证                                                |
...
| 11  | DCO 溯源：main 上 **9 个提交缺少 `Signed-off-by`**（8 个分叉压缩提交，加 PR #8 的 squash 提交 `55eb7a1`——GitHub squash 重写作者并丢弃 trailer） | `check-dco.mjs` 已如实报告；未代签                     | 由真实作者本人补签（`git commit --amend -s` 后 force-push，或由本人重新 squash）；今后合并使用 merge commit 保留签名 | 所有者（作者本人） |
...
| 社区贡献（未来）                | 按 DCO + Apache-2.0 提交的公共代码 | 机制已就绪（CONTRIBUTING.md/CI）；**尚无已接收的 DCO 贡献**；贡献者保留版权                         |
| Fork 工程（本分支已完成的工作） | 品牌/遥测替换/打包/合规脚本等      | 作者是 `hong@cloudjetkpi.com`（5 个提交，见 git 历史）；**尚未核实**该作者的雇佣/委托关系与成果归属 |
...
| #   | 事项                                                             | 现状                                             | 需要的证据/行动                                                                                   |
| --- | ---------------------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------- |
| O1  | fork 工程作者（`hong@cloudjetkpi.com`）的雇佣/委托与成果归属     | **未核实**；README/COMPLIANCE 均未做公司归属声明 | 书面雇佣/委托协议或版权转让；确认无前雇主权利残留；在此之前不添加任何 RunLumi/CLOUDJET 的版权声明 |
| O2  | RunLumi / Lumi / CLOUDJET SOLUTIONS PTE. LTD. 与本项目的法律关系 | 未在仓库中声明（本清单不构成声明）               | 由所有者出具书面确认：谁有权以实体名义商用、签约、注册商标                                        |
...
| O7  | DCO 首批贡献                                                     | 尚无                                             | 首个外部 DCO 贡献落地后，在本文档记录其 PR 与签名核验结果                                         |
